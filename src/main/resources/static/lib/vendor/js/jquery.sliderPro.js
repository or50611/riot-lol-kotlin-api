	...
	var Thumbnails = {
		...
		_thumbnailsOnUpdate: function() {
			...
			if ( this.$thumbnails === null ) {
				if ( this.$slider.find( '.thumbnails' ).length !== 0 ) {
					this.$thumbnails = this.$slider.find( '.thumbnails' ).appendTo( this.$thumbnailsContainer );

					// Shuffle/randomize the thumbnails
					if ( this.settings.shuffle === true ) {
						var thumbnails = this.$thumbnails.find( '.thumbnail' ),
							shuffledThumbnails = [];
					...
					this.$thumbnails = $( '<div class="thumbnails"></div>' ).appendTo( this.$thumbnailsContainer );
				}
			}
			...
			if ( this.thumbnailsOrientation === 'horizontal' ) {
				this.$thumbnails.css({ 'width': '100%', 'height':'80px' });
